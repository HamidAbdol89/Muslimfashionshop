...
@section('content')
    <div class="container my-5">
        <img src="{{ asset('images/bannerman.jpg') }}" alt="Banner man" class="w-full h-auto rounded">
                ...
                <div class="tags-list">
                    @foreach ($tags as $tag)
                        @if (strlen($tag->name) > 1 && !is_numeric($tag->name))
                            ...
                            <a href="{{ route('frontend.shop.tag', ['tagId' => $tag->id]) }}" class="tag-item"
                                data-id="{{ $tag->id }}">
                                {{ $tag->name }}
                            </a>
                        @endif
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#product-list').html(response
                        .html
                        ); // Giả sử bạn có một phần tử có id "product-list" để chứa danh sách sản phẩm
