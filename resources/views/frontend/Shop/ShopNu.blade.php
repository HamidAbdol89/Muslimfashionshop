...
@section('content')
    <div class="container my-5">
        <img src="{{ asset('images/bannergirl.jpg') }}" alt="Banner girl" class="w-full h-auto rounded">
                ...
                <div class="tags-list">
                    @foreach ($tags as $tag)
                        @if (strlen($tag->name) > 1 && !is_numeric($tag->name))
                            ...
                            <a href="{{ route('frontend.shop.ShopByTagForWomen', ['tagId' => $tag->id]) }}" class="tag-item"
                                data-id="{{ $tag->id }}">
                                {{ $tag->name }}
                            </a>
                        @endif
